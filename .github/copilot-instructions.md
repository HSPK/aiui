...
- **Transport vs Schema 两轴**：`resolveModel` 返回 `{transport, schema}` 两个 adapter。**transport**（来自 `provider.adapter_id`）拥有 URL/auth/headers/响应解析；**schema**（默认沿用 transport，可被 `model.schema_adapter_id` 覆盖）拥有请求体形状——`accepted_fields`/`rejected_fields`/`selectUpstreamApi`/`transformRequest`。用例：proxy 把 Foundry 包成 OpenAI URL，但上游仍按 Foundry 拒绝 `stream_options`——这时 provider 用 `openai` adapter，model 用 `schema_adapter_id: "azure-foundry"`。**写 gateway 新分支前先想能不能用这个 split 解决**
