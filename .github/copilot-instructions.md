...
- **关闭自动注入**：`stream_options.include_usage` 默认自动注入到流式 chat 请求。要禁用：在 provider/model `default_params` 里设 `stream_options: null`，gateway 看到 null 会从最终 body 里删除它（见 `maybeInjectStreamUsage`）
