...
- **Transport vs Schema 两轴**：`resolveModel` 返回 `{transport, schema}` 两个 adapter。**transport** = `provider.adapter_id`，拥有 URL/auth/headers/响应解析。**schema** 三级回落：`model.schema_adapter_id` → `provider.schema_adapter_id` → transport；拥有请求体——`accepted_fields`/`rejected_fields`/`selectUpstreamApi`/`transformRequest`。典型场景：proxy 把 Foundry 包成 OpenAI URL → provider 用 `openai` adapter + `schema_adapter_id: azure-foundry`（全部 model 自动按 Foundry 过滤）；某个 model 个例可在 model 行上再覆盖。**写 gateway 新分支前先想能不能用这个 split 解决**
